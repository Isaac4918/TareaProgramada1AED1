...
public class Controller1 implements Serializable {
    ...
    static ListaEnlazadaPalabra linkedlist=new ListaEnlazadaPalabra();
    public Abecedario abc = new Abecedario();

    public Controller1() {
    }

    public Abecedario getAbc() {
        return abc;
    }

    public void setAbc(Abecedario abc) {
        this.abc = abc;
    }
    ...
    public void setTiles(Text tileletter, Text tilevalue) {
        Letra tmp = tl.generatetiles(this.abc);
        tileletter.setText(tmp.getLetra());
        tilevalue.setText(String.valueOf(tl.settilevalue(tmp)));
    ...
    public static ArrayList listaPalabras(){
        ArrayList listaPalabra = new ArrayList();
        Letra aux=linkedlist.first; //aux apunta al primero
        while(aux!=null){
            listaPalabra.add(aux.getLetra()+", "+aux.getPosx()+", "+aux.getPosy());
            aux=aux.getNext();
        }
        System.out.println("**************************************");
    ...
    private void texto(String str, double layoutX, double layoutY) {
        ...
        for(int i = 0; i<= abc.ListaLetras.length; i++){
            if(this.abc.ListaLetras[i].getLetra() == str.toLowerCase()){
                L1 = this.abc.ListaLetras[i];
                break;
            }
        }
        L1.setPosx(valorx);
